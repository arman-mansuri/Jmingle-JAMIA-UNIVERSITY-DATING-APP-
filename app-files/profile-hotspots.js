...
// Used whenever a Supabase row has no photo1_url — avoids setting
// img.src = null, which the browser turns into a broken request for a
// literal file named "null" (that's the "null:1 — 404" console errors).
var PLACEHOLDER_IMAGE = 'data:image/svg+xml;utf8,' + encodeURIComponent(
  '<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 400 500">' +
  '<rect width="400" height="500" fill="#2a2a2a"/>' +
  '<circle cx="200" cy="190" r="70" fill="#555"/>' +
  '<path d="M60 460 Q200 320 340 460 Z" fill="#555"/>' +
  '</svg>'
);
...
function mapSupabaseRowToProfile(row) {
  return {
    ...
    image: resolveImageUrl(row.photo1_url) || PLACEHOLDER_IMAGE
  };
}
...
function setupProfileHotspots(scenes) {
  ...
  Object.keys(grouped).forEach(function(location) {
    var hasLocation = location && location !== 'null';
    var pin = hasLocation ? locationPins[location] : null;
    if (!pin) {
      if (hasLocation) {
